...
--
-- NÃO APLICADO. Escrito a partir das funções como estão em produção
-- (pg_get_functiondef em 25/09) e revisado; aplicar pelo SQL Editor numa
-- transação só, ANTES de publicar as funções social-meta-oauth e mesa-ads
-- novas (elas funcionam sem este SQL, mas os campos novos só aparecem depois
-- dele). Rodar de novo não faz mal.
--
-- O QUE O DONO RELATOU e o que os dados mostraram (consultas só de leitura):
--
-- 1. "Adicionar conta de anúncio está bugado, não puxa o quanto eu adicionei."
--    a) A leitura da conta (kind 'account') pedia só name,currency,
--       account_status,timezone_name e gravava SÓ o nome. Gasto total, saldo,
--       limite de gasto e meio de pagamento nunca foram pedidos à Meta.
--    b) A tela marca "lida" pela última linha DIÁRIA de campanha. Conta nova
--       ou sem gasto no mês (5 de 13: Terra Flor, Thainá, SKC, Mirante 2 e
--       Acerbi sem números) nunca tem linha, então fica "aguardando a
--       primeira leitura" para sempre e não aparece no hub de /anuncios.
--    c) A fila dos criativos tinha um pedido órfão da Preserva Eco desde
--       01/09 (attempts = 3, sem requisição): a fase B só redespacha com
--       attempts < 3, a fase A só lê quem tem requisição, e a fase C pula a
--       conta que tem pedido na fila. Os números por anúncio da Preserva
--       pararam em 31/08.
--    d) Um token global só: conectar pela Meta com OUTRO perfil revogava o
--       token anterior, e as contas que só o perfil anterior enxergava
--       passavam a falhar em silêncio (3 tentativas e o pedido some).
--    e) Janela fixa de 30 dias e o resultado do painel só contava mensagem,
--       cadastro e compra: campanha de tráfego (14 no banco) e de
--       reconhecimento aparecia com zero resultado.
--
-- 2. "Não está puxando os resultados das métricas do perfil do Instagram."
--    a) O mergulho por post (alcance, salvamentos, compartilhamentos) lia
--       cada post UMA vez e nunca mais: 42 de 158 posts foram lidos na
--       primeira hora depois de publicados e ficaram congelados ali.
--    b) O retrato da semana corrente pede `until` = amanhã; a Meta devolve
--       lista vazia e o motor gravava alcance 0 (todas as contas na semana
--       de 21/09: alcance 0, interações e visitas vazias).
--    c) SEGURANÇA: a resposta da Meta era gravada inteira em
--       social_metrics_weekly.raw, com paging.previous, que traz o token da
--       página na URL. 85 de 85 linhas tinham token, e a tabela é legível
--       pela equipe e pelo próprio cliente (RLS de leitura). Este SQL limpa
--       as linhas e para de gravar paging. Recomendado: gerar token novo das
--       páginas (reconectar) depois de aplicar.
...
-- select count(*) from public.social_metrics_weekly where raw::text like '%access_token=%';  -- 0
-- select external_account_id, coletado_em, amount_spent, saldo_disponivel, erro from public.ads_account_snapshot;
-- select public.collect_ads_now();  -- como equipe, pelo painel
