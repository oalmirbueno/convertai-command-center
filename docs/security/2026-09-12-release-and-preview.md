...
O cenário do cliente também revelou que a área transparente do logo, maior que a barra fixa, interceptava o botão “Voltar aos projetos” logo abaixo dela. A contenção fica somente no wrapper do logo, preservando a escala da marca e os menus. O teste continua exigindo um clique normal no botão; não usa clique forçado para ocultar a sobreposição.
...
Na implantação aprovada, os workflows de banco e de release MCP exigem o SHA completo da ponta atual de `main`, após o merge revisado. O SHA do draft identifica a revisão deste pacote, mas não substitui essa conferência da origem de implantação.
...
4. Após aprovação e preflights, implantar individualmente `brain-client-context` e `notify-admin` do checkout revisado, usando `supabase functions deploy <nome> --project-ref jjjtkowvxemvituvywvf` e preservando `verify_jwt = true` do config. A segunda função inclui o retorno correto das falhas de notificação. Esse lote de duas funções não corresponde à lista do workflow público nem ao lote completo do workflow interno. Implantar `mcp-server` e `mcp-oauth-metadata` pelo workflow próprio, do mesmo checkout preparado. Esse workflow gera SHA/digest compartilhado e falha se as duas pontas não concordarem. Rollback para código legado continua disponível, sem exigir um campo de identidade que aquela versão ainda não possuía.
