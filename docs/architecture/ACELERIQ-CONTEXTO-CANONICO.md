...
8. Aplicar migrations pelo workflow GitHub forward-only, somente no tip atual de `main`, depois de confirmar backup restaurável e checklist privado real; nunca usar reset, seed, repair, `--include-all` ou escrita manual na ledger de produção.
...
- A produção Lovable possui baseline normalizado e fail-closed: manifesto explícito de 96 aliases do histórico remoto anterior a `20260807210000`, duas migrations absorvidas somente por schema attestation e 19 migrations forward-only atuais fixadas individualmente a partir do corte, das quais 12 possuem aliases registrados pelo runner. Cada migration futura posterior entra explicitamente nesse manifesto no mesmo PR. Esse baseline é verificação somente leitura, não altera `schema_migrations` e não dispensa backup restaurável nem checklist privado confirmados com evidência real.
