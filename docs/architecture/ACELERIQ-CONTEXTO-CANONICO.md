...
- Backend atual: Supabase/Postgres `jjjtkowvxemvituvywvf`; painel `https://aceleriq.online`.
- Atualização confirmada por Almir em 12/09/2026: a migração do projeto anterior `gicbrgagstyvbaaumprj` para o atual já ocorreu. A data exata e o procedimento dessa migração não foram auditados neste lote. Referências ao projeto anterior em artefatos legados são históricas e não definem o destino de uma nova implantação. Lovable continua no fluxo do frontend e da compatibilidade.
...
- O manifesto normalizado do projeto anterior contém 96 aliases do histórico remoto anterior a `20260807210000`, duas migrations absorvidas por schema attestation e migrations forward fixadas individualmente, incluindo 12 aliases do runner. As contagens antigas documentadas eram uma fotografia histórica; esse manifesto não representa automaticamente o ledger do backend atual.
- O ledger de `jjjtkowvxemvituvywvf` foi inspecionado somente em leitura em 12/09/2026: 206 entradas existentes, das quais 201 não retêm SQL original. O snapshot por projeto em `supabase/deployment-baselines/` detecta drift de metadados e preserva essas entradas; não comprova replay do SQL histórico. O preflight MCP exige esse snapshot mais as duas correções incrementais de 12/09. Não copiar, reparar ou reexecutar o histórico do projeto anterior para satisfazer o preflight. Aprovação explícita, backup restaurável e checklist real continuam obrigatórios. O plano deste lote está em [`2026-09-12-release-and-preview.md`](../security/2026-09-12-release-and-preview.md).
