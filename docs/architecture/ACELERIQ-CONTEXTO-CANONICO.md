...
- A produção Lovable possui baseline normalizado e fail-closed: manifesto explícito de 96 aliases do histórico remoto anterior a `20260807210000`, duas migrations absorvidas somente por schema attestation e 12 migrations forward-only atuais fixadas individualmente a partir do corte. Cada migration futura posterior entra explicitamente nesse manifesto no mesmo PR. Esse baseline é verificação somente leitura, não altera `schema_migrations` e não dispensa backup restaurável nem checklist privado confirmados com evidência real.
