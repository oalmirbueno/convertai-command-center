...
export function buildClientProjectFields(opts: {
  ...
  /** Strategic, client-facing summary generated by the assistant (3-5 sentences). */
  clientSummary?: string | null;
  plan?: PlanLike;
}) {
  const plan = opts.plan?.milestones || [];
  const taskTotal = plan.reduce((sum, m) => sum + (m.tasks?.length || 0), 0);
  const summary = sanitizeClientText(opts.clientSummary);
  const intro = summary || TYPE_INTRO[opts.type || "other"] || TYPE_INTRO.other;
