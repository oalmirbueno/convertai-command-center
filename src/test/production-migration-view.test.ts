...
describe("production migration view", { timeout: 30000 }, () => {
  ...
  it("validates the explicit 96 legacy + 2 attestations + 151 forward + 12 alias contract", () => {
    const plan = loadProductionMigrationPlan();
    ...
    expect(plan.manifest.forward_migrations).toHaveLength(151);
    expect(plan.forwardMigrations).toHaveLength(151);
    ...
    expect(plan.forwardLedger).toHaveLength(151);
    expect(versions).toHaveLength(247);
    ...
    expect(unaliased).toHaveLength(139);
  ...
  it("declares an exact remote hash mode per forward migration and uses raw ledger hashes", () => {
    ...
    expect(cliSplit).toHaveLength(146);
  ...
  it("accepts the complete declared 247-row raw ledger and rejects normalized rows", () => {
    const plan = loadProductionMigrationPlan();
    const rows = remoteRows(151);
    ...
    expect(reconciliation.appliedForward).toHaveLength(151);
  ...
  it("emits legacy sentinels, omits attestations, and copies pending forward SQL byte-for-byte", () => {
    ...
    expect(filenames).toHaveLength(247);
  ...
  it("reconciles the complete aliased ledger to sentinels only and zero pending forward", () => {
    const plan = loadProductionMigrationPlan();
    const { outputDir, result } = buildFixture(151);
    const filenames = readdirSync(outputDir).sort();
  ...
  it("keeps only the fifty-five new forwards pending over the old 192-row baseline without replaying history", () => {
    const plan = loadProductionMigrationPlan();
  ...
  it("turns an applied forward prefix into sentinels and leaves only its suffix pending", () => {
    ...
    expect(result.pendingForward).toBe(149);
  ...
  it("supports the workflow source-directory override", () => {
    ...
    expect(listProductionVersions({ sourceDir })).toHaveLength(247);
  });
  ...
  it("keeps a future unaliased forward migration pending after the current package", () => {
    const plan = loadProductionMigrationPlan();
    const rows = parseRemoteLedgerCsv(ledgerCsv(remoteRows(151)));
  ...
  it("keeps a future supabase_cli_split forward pending until its split hash lands", () => {
    ...
    const live = parseRemoteLedgerCsv(ledgerCsv(remoteRows(151)));
