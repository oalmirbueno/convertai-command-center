...
describe("production migration view", { timeout: 30000 }, () => {
  ...
  it("validates the explicit 96 legacy + 2 attestations + 159 forward + 12 alias contract", () => {
    const plan = loadProductionMigrationPlan();
    ...
    expect(plan.manifest.forward_migrations).toHaveLength(159);
    expect(plan.forwardMigrations).toHaveLength(159);
    ...
    expect(plan.forwardLedger).toHaveLength(159);
    expect(versions).toHaveLength(255);
    ...
    expect(unaliased).toHaveLength(147);
  ...
  it("declares an exact remote hash mode per forward migration and uses raw ledger hashes", () => {
    ...
    expect(cliSplit).toHaveLength(154);
  ...
  it("accepts the complete declared 255-row raw ledger and rejects normalized rows", () => {
    const plan = loadProductionMigrationPlan();
    const rows = remoteRows(159);
    ...
    expect(reconciliation.appliedForward).toHaveLength(159);
  ...
  it("emits legacy sentinels, omits attestations, and copies pending forward SQL byte-for-byte", () => {
    ...
    expect(filenames).toHaveLength(255);
  ...
  it("reconciles the complete aliased ledger to sentinels only and zero pending forward", () => {
    const plan = loadProductionMigrationPlan();
    const { outputDir, result } = buildFixture(159);
    const filenames = readdirSync(outputDir).sort();
  ...
  it("keeps only the sixty-three new forwards pending over the old 192-row baseline without replaying history", () => {
    const plan = loadProductionMigrationPlan();
  ...
  it("turns an applied forward prefix into sentinels and leaves only its suffix pending", () => {
    ...
    expect(result.pendingForward).toBe(157);
  ...
  it("supports the workflow source-directory override", () => {
    ...
    expect(listProductionVersions({ sourceDir })).toHaveLength(255);
  });
  ...
  it("keeps a future unaliased forward migration pending after the current package", () => {
    const plan = loadProductionMigrationPlan();
    const rows = parseRemoteLedgerCsv(ledgerCsv(remoteRows(159)));
  ...
  it("keeps a future supabase_cli_split forward pending until its split hash lands", () => {
    ...
    const live = parseRemoteLedgerCsv(ledgerCsv(remoteRows(159)));
