...
describe("production migration view", { timeout: 30000 }, () => {
  ...
  it("validates the explicit 96 legacy + 2 attestations + 113 forward + 12 alias contract", () => {
    const plan = loadProductionMigrationPlan();
    ...
    expect(plan.manifest.forward_migrations).toHaveLength(113);
    expect(plan.forwardMigrations).toHaveLength(113);
    ...
    expect(plan.forwardLedger).toHaveLength(113);
    expect(versions).toHaveLength(209);
    ...
    expect(unaliased).toHaveLength(101);
  ...
  it("declares an exact remote hash mode per forward migration and uses raw ledger hashes", () => {
    ...
    expect(cliSplit).toHaveLength(108);
  ...
  it("accepts the complete declared 209-row raw ledger and rejects normalized rows", () => {
    const plan = loadProductionMigrationPlan();
    const rows = remoteRows(113);
    ...
    expect(reconciliation.appliedForward).toHaveLength(113);
  ...
  it("emits legacy sentinels, omits attestations, and copies pending forward SQL byte-for-byte", () => {
    ...
    expect(filenames).toHaveLength(209);
  ...
  it("reconciles the complete aliased ledger to sentinels only and zero pending forward", () => {
    const plan = loadProductionMigrationPlan();
    const { outputDir, result } = buildFixture(113);
    const filenames = readdirSync(outputDir).sort();
  ...
  it("keeps only the seventeen new forwards pending over the old 192-row baseline without replaying history", () => {
    const plan = loadProductionMigrationPlan();
    const oldRows = remoteRows(96);
    expect(oldRows).toHaveLength(192);
    const reconciliation = validateRemoteLedger(plan, parseRemoteLedgerCsv(ledgerCsv(oldRows)));
    expect(reconciliation.appliedForward).toHaveLength(96);
    expect(reconciliation.appliedAliases).toHaveLength(12);
    expect(reconciliation.pendingForward.map((entry) => entry.version)).toEqual([
      "20260912213530", "20260912213638", "20260914174905", "20260916100000", "20260916120000", "20260916150000", "20260917200000", "20260918120000", "20260918130000", "20260918140000", "20260918150000", "20260918160000", "20260921120000", "20260921130000", "20260921140000", "20260922120000", "20260922122000",
    ]);

    const { outputDir, result } = buildFixture(96);
    expect(result).toEqual({ aliases: 96, appliedForward: 96, appliedAliases: 12, pendingForward: 17, files: 209 });
    ...
    for (const applied of plan.forwardLedger.slice(0, 96)) {
      const filename = applied.alias
        ? `${applied.version}_production_ledger_sentinel.sql`
        : applied.canonical.filename;
      const emitted = readFileSync(join(outputDir, filename), "utf8");
      expect(emitted).toContain(`migration sentinel ${applied.version}`);
      expect(emitted).toContain("RAISE EXCEPTION");
    }
    for (const pending of reconciliation.pendingForward) {
      expect(readFileSync(join(outputDir, pending.filename))).toEqual(pending.bytes);
    }
    const executableFiles = readdirSync(outputDir).filter((filename) =>
      !readFileSync(join(outputDir, filename), "utf8").includes("migration sentinel"));
    expect(executableFiles.sort()).toEqual(reconciliation.pendingForward.map((entry) => entry.filename).sort());
  }, TEMPO_PACOTE_COMPLETO);

  it("preserves the prior 194-row declared ledger and leaves Central, the review notice and the Hermes review pending", () => {
    const plan = loadProductionMigrationPlan();
    ...
    expect(reconciliation.appliedAliases).toHaveLength(12);
    expect(reconciliation.pendingForward.map((entry) => entry.version)).toEqual(["20260914174905", "20260916100000", "20260916120000", "20260916150000", "20260917200000", "20260918120000", "20260918130000", "20260918140000", "20260918150000", "20260918160000", "20260921120000", "20260921130000", "20260921140000", "20260922120000", "20260922122000"]);
  });
  ...
  it("turns an applied forward prefix into sentinels and leaves only its suffix pending", () => {
    ...
    expect(result.pendingForward).toBe(111);
  ...
  it("supports the workflow source-directory override", () => {
    ...
    expect(listProductionVersions({ sourceDir })).toHaveLength(209);
  });
  ...
  it("keeps a future unaliased forward migration pending after the current package", () => {
    const plan = loadProductionMigrationPlan();
    const rows = parseRemoteLedgerCsv(ledgerCsv(remoteRows(113)));
    ...
    const syntheticPlan = {
      ...
      forwardLedger: [
        ...plan.forwardLedger,
        {
          canonical: { ...future.canonical, version: "20260915120000" },
          alias: null,
  ...
  it("keeps a future supabase_cli_split forward pending until its split hash lands", () => {
    ...
    const live = parseRemoteLedgerCsv(ledgerCsv(remoteRows(113)));
