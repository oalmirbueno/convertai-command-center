...
import { FLUTUANTE, ICONES, PAINEL, useRodaPresa } from "./comum";
...
/**
 * Galeria de modelos prontos. `compacta`: dentro da gaveta do quadro (a
 * gaveta já tem o título e o fechar), em 2 colunas e miniatura baixa; sem ela,
 * o painel largo do modo lista.
 */
export function GaleriaDeModelos({ onAplicar, onFechar, onMontarPeloContexto, compacta = false }: { onAplicar: (chave: string) => void; onFechar?: () => void; onMontarPeloContexto?: () => Promise<unknown>; compacta?: boolean }) {
  const { catalogo } = useMesa();
  return (
    <div className={compacta ? "min-w-0" : `${PAINEL} min-w-0 rounded-2xl p-3`} data-modelos-prontos="">
      <div className="mb-2 flex min-w-0 flex-wrap items-center">
        {!compacta && (
          <>
            <Wand2 className="mr-1.5 h-3.5 w-3.5 text-emerald-300" />
            <p className="min-w-0 flex-1 text-[12px] font-semibold">Modelos prontos: montam o quadro em 1 clique</p>
          </>
        )}
        {compacta && <p className="mb-1.5 w-full text-[11px] leading-snug text-zinc-400">Montam o quadro em 1 clique, já ligado ao Resultado.</p>}
      ...
      <div className={compacta ? "grid min-w-0 grid-cols-2 gap-1.5" : "grid min-w-0 grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5"}>
        {MODELOS_PRONTOS.map((m) => (
          <button key={m.chave} type="button" onClick={() => onAplicar(m.chave)} data-modelo-pronto={m.chave} title={m.dica} className="min-w-0 rounded-xl border border-white/10 bg-white/[0.03] p-1.5 text-left transition-colors hover:border-emerald-400/60">
            <MiniaturaDoModelo m={m} altura={compacta ? 64 : 84} />
...
/**
 * Como funciona. No quadro, um cartãozinho encostado na paleta (não mais no
 * meio, por cima dos cartões); no modo lista, no fluxo da página.
 */
export function ComoFunciona({ onFechar, noQuadro = true }: { onFechar: () => void; noQuadro?: boolean }) {
  const roda = useRodaPresa<HTMLDivElement>();
  return (
    <div
      ref={roda}
      className={`${PAINEL} ${FLUTUANTE} rounded-2xl p-3 ${noQuadro ? "absolute left-[76px] right-2 top-3 z-10 sm:right-auto sm:w-[300px]" : "relative w-full min-w-0"}`}
      data-como-funciona=""
    >
