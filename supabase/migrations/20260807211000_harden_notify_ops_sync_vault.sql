-- Re-apply the portable Ops sync trigger after the historical migrations.
-- The older migration files remain sanitized for fresh environments, while
-- this forward-only migration updates projects where those versions ran before
-- the Vault-backed definition was introduced.
...
BEGIN
  ...
  -- Missing, blank, or unreadable Vault entries keep the retired bridge off.
  -- No project-specific endpoint or credential is embedded in the function.
  BEGIN
