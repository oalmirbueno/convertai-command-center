-- Fronteira transacional para papéis gerenciados pelo painel.
-- A função é invoker e só pode ser chamada pela service_role da Edge Function.

-- Fecha as rotas antigas de escrita direta via Data API. A leitura continua
-- regida pelas policies existentes, mas toda mutação passa pela Edge Function.
...
-- O produto trabalha com exatamente um papel gerenciado por usuário. Se houver
-- legado ambíguo, a implantação para com uma mensagem clara em vez de escolher
-- silenciosamente qual privilégio deve prevalecer.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
...
BEGIN
  ...
  -- Serializa mudanças administrativas para impedir que duas requisições
  -- rebaixem simultaneamente os últimos administradores. O lock é liberado
  -- automaticamente no commit ou rollback desta RPC.
  ...
  -- DELETE + INSERT pertencem à mesma transação da RPC. Se o INSERT falhar,
  -- o papel anterior é restaurado automaticamente pelo PostgreSQL.
