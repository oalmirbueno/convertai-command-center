-- Conserto: o cliente vive em profiles, nao numa tabela `clients`.
--
-- A migration 20260828010000 gravou `select name from public.clients`. Essa
-- tabela nao existe neste banco: projects.client_id aponta para profiles.id.
--
-- Passou despercebido porque plpgsql nao resolve nome de tabela ao criar a
-- funcao, so na primeira execucao. Entao o SQL rodou limpo, a conferencia deu
-- certo, e a bomba ficou armada no unico caminho que a camada existe para
-- servir: o `done` com evidencia. Pior: como a excecao derruba a transacao
-- inteira, o agente perderia tambem a run e a auditoria daquele relato.
--
-- Nao houve estrago em dado nenhum: ninguem chegou a reportar done ainda.
--
...
begin
  ...
  if _status_novo = 'done' and _kanban_task_id is not null then
    ...
    if _client_id is not null then
      -- O cliente e um profile, nao uma tabela `clients`: projects.client_id
      -- aponta para profiles.id. Eu tinha escrito `from public.clients` de
      -- cabeca, sem conferir, e a tabela nunca existiu. plpgsql so resolve
      -- nome de tabela na PRIMEIRA EXECUCAO, entao a funcao foi criada sem
      -- reclamar e ia estourar exatamente no caminho que importa: o done com
      -- evidencia. E como o erro derruba a transacao inteira, nem a run nem a
      -- auditoria seriam gravadas. O nome segue a mesma regra da tela:
      -- empresa quando existe, pessoa quando nao.
