-- EXPAND phase for public bearer credentials. Private digests and v2 state are
-- introduced alongside the legacy plaintext columns so the old and new Edge
-- runtimes can coexist. Plaintext removal and legacy-contract replacement must
-- happen only in a later, separately validated CUTOVER/CONTRACT migration.
...
-- Safe on a resumed deployment: never replace a live private claim or collide
-- with a digest that was already created by a dual-write.
ON CONFLICT DO NOTHING;

-- Preserve only active draft quiz invitations. Normalizing before ranking and
-- hashing keeps the private lookup consistent with both Edge runtimes, which
-- trim URL tokens before use. Whitespace-equivalent duplicates fail closed in
-- v2 while their untouched legacy rows remain available during EXPAND.
...
-- EXPAND invariant: public plaintext columns and their lookup indexes are left
-- untouched so the previously deployed Edge functions remain functional.

-- EXPAND dual-write bridge. Legacy columns remain authoritative for the old
-- runtime, while every UPDATE that issues or advances a token mirrors its
-- digest and state privately. This is deliberately UPDATE-only: normal user
-- creation inserts the profile first and issues a token afterward, avoiding a
-- child-FK write from a BEFORE INSERT trigger before the parent row exists.
...
BEGIN
  IF NEW.first_access_token IS NOT NULL
    ...
    -- A normalized duplicate is intentionally legacy-only. Do not reject the
    -- public write during EXPAND, but revoke any private mapping for the
    -- ambiguous bearer so neither row can authenticate through v2.
    IF EXISTS (
      SELECT 1
    ...
    IF NEW.first_access_token IS DISTINCT FROM OLD.first_access_token THEN
      ...
      EXCEPTION
        WHEN unique_violation THEN
          -- Close the v2 mapping if two legacy writers concurrently converge on
          -- the same normalized bearer; the public write remains compatible.
          UPDATE app_private.first_access_tokens AS token_state
          SET
            status = CASE
              WHEN token_state.status = 'used' THEN 'used'
    ...
    IF NOT FOUND THEN
      ...
      EXCEPTION
        WHEN unique_violation THEN
          -- A pre-existing duplicate plaintext bearer cannot be represented by
          -- the unique v2 digest. Leave that legacy row operational but do not
          -- make the ambiguous bearer valid through the v2 lookup.
          NULL;
      END;
      RETURN NEW;
    END IF;
  ...
  -- Clearing a legacy bearer is a real state transition, not a cutover scrub.
  -- A v2 consume marks the private row used before clearing the public token;
  -- an old-runtime success marks it used here; an administrative clear revokes.
  IF OLD.first_access_token IS NOT NULL
    ...
    EXCEPTION
      WHEN unique_violation THEN
        -- As above, an ambiguous duplicate remains a legacy-only bearer. Its
        -- normal clear/consume transition must not be blocked by private state.
        NULL;
    END;
  END IF;
...
BEGIN
  ...
  -- The public write is intentional during EXPAND: it keeps the old lookup
  -- contract alive and the UPDATE trigger atomically dual-writes the digest.
...
BEGIN
  ...
  -- Match the legacy lock order. Once a failed v2 claim is released, mirror its
  -- attempt metadata publicly before another v1 request can read stale values.
  PERFORM 1
...
BEGIN
  ...
  -- Old runtime updates lock profiles first and reach the private row through
  -- the dual-write trigger. Use the same lock order here to avoid a mixed-
  -- runtime deadlock during the EXPAND window.
  PERFORM 1
  FROM public.profiles AS profile
  WHERE profile.id = _profile_id
  FOR UPDATE;
  ...
  IF _profile_id IS NULL THEN
    -- A concurrent retry may have finalized while this call waited for the
    -- public-row lock.
    RETURN EXISTS (
      SELECT 1
      FROM app_private.first_access_tokens AS t
...
-- Keep legacy quiz counters, terminal status and the private digest in sync.
-- AFTER INSERT is FK-safe because the public submission already exists.
...
BEGIN
  IF _raw_token IS NULL OR _raw_token = '' THEN
    IF TG_OP = 'UPDATE' THEN
      -- Clearing the legacy lookup value is a revocation event, not a cutover.
      -- Keep the old public write valid while making the prior v2 digest terminal.
  ...
  -- Exact tokens are already unique publicly, but whitespace-equivalent legacy
  -- values are not. Keep both raw rows and close every ambiguous v2 digest.
  IF EXISTS (
    SELECT 1
    FROM public.quiz_submissions AS duplicate_submission
  ...
  EXCEPTION
    WHEN unique_violation THEN
      -- Whitespace-equivalent or exact duplicate legacy bearers stay usable by
      -- the old raw-token runtime, but the ambiguous row is never opened in v2.
      UPDATE app_private.quiz_invitation_tokens AS current_token
      SET
        used_at = COALESCE(current_token.used_at, _used_at, now()),
        updated_at = now()
...
-- Shared private issuer used by both the unchanged v1 scalar contract and the
-- richer v2 contract. The public row intentionally stores the raw bearer until
-- a later CUTOVER so the old submit-quiz runtime keeps working.
...
BEGIN
  -- Preserve the v1 authorization contract: every established staff role can
  -- issue a link from the shared dashboard, not only admins and managers.
  ...
  -- Serialize the per-actor quota so concurrent issue requests cannot all
...
BEGIN
  ...
  -- Legacy writes lock the public row before their sync trigger reaches the
  -- private row. Revalidate and lock in that same order for mixed runtimes.
  SELECT t.* INTO _token
  FROM app_private.quiz_invitation_tokens AS t
