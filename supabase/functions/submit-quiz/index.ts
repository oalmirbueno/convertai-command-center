...
const REVENUE_SCORE: Record<string, number> = {
  "Até R$ 20k/mês": 20, "R$ 20k-50k/mês": 35, "R$ 50k-200k/mês": 55,
  "R$ 200k-500k/mês": 75, "R$ 500k-1M/mês": 85, "R$ 1M-5M/mês": 95, "R$ 5M+/mês": 100,
};
const TEAM_SCORE: Record<string, number> = {
  "Solo (1 pessoa)": 30, "2-5 pessoas": 50, "6-15 pessoas": 75,
  "16-50 pessoas": 90, "51-200 pessoas": 95, "200+": 100,
};
...
function calculateICP(body: Record<string, string | undefined>): { score: number; plan: string } {
  ...
  const commitment = ["positioning","differential","icp","main_pains","goals_12m","success_metric"]
    .filter(k => (body[k] ?? "").trim().length > 20).length / 6 * 100;
...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: err instanceof Error ? err.message : "erro" }), { status: 500, headers: cors });
  }
});
