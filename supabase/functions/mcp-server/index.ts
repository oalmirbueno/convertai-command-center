...
const MCP_ALLOWED_ORIGINS = resolveMcpAllowedOrigins(
...
Deno.serve(async (req) => {
  if (!isMcpOriginAllowed(req, MCP_ALLOWED_ORIGINS)) {
    return jsonResponse(
      rpcError(null, RpcErrors.forbidden, 'Origin is not allowed'),
      403,
    );
  }
  ...
  if (!isMcpProtocolVersionSupported(req)) {
    return jsonResponse(
      rpcError(null, RpcErrors.invalidRequest, 'Unsupported MCP-Protocol-Version'),
      400,
    );
  }
  ...
  try {
    ...
    if (error instanceof McpRequestBodyTooLargeError) {
      return jsonResponse(
        rpcError(null, RpcErrors.invalidRequest, 'Request body is too large'),
        413,
      );
    }
  ...
  if (!validated.ok) {
    return jsonResponse(
      rpcError(validated.id, validated.code, validated.message),
      400,
    );
  }
  ...
  if (!auth.ok) {
    if (!isPublicRpc(message)) {
      ...
      const payload = toolChallengeOnly && message.method === 'tools/call'
