...
var define_import_meta_env_default = { VITE_SUPABASE_PROJECT_ID: "gicbrgagstyvbaaumprj", VITE_SUPABASE_URL: "https://gicbrgagstyvbaaumprj.supabase.co", MODE: "production", BASE_URL: "/", DEV: false, PROD: true, SSR: false };
...
var health_default = defineTool({
  ...
  handler: (_input, ctx) => {
    ...
    return {
      content: [{
        type: "text",
        text: authed ? `ok \xB7 autenticado como ${ctx.getUserEmail() ?? ctx.getUserId()}` : "ok \xB7 sem autentica\xE7\xE3o"
      }],
      structuredContent: {
        status: "ok",
...
var list_tasks_default = defineTool4({
  ...
  description: "Lista tarefas de projetos ativos vis\xEDveis ao usu\xE1rio autenticado (RLS aplicado). Inclua projetos arquivados explicitamente para consultar o hist\xF3rico; project_archived identifica a origem hist\xF3rica.",
  inputSchema: {
    ...
    include_archived_projects: z3.boolean().optional().describe("Inclui tarefas de projetos arquivados dentro do mesmo acesso. Padr\xE3o: false."),
...
var get_client_metrics_default = defineTool9({
  ...
  inputSchema: {
    client_id: z8.string().uuid().describe("Cliente dono da conta."),
    weeks: z8.number().int().min(1).max(52).optional().describe("Semanas de hist\xF3rico (padr\xE3o 8)."),
    include_posts: z8.boolean().optional().describe("Incluir desempenho por publica\xE7\xE3o (padr\xE3o true).")
  },
  annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false },
  handler: async ({ client_id, weeks, include_posts }, ctx) => {
    ...
    if (guard) return guard;
    const sb = supabaseForUser(ctx);
    const { data: weekRows, error: weekError } = await sb.from("social_metrics_weekly").select("week_start, week_end, followers, media_count, reach, profile_views, accounts_engaged, total_interactions, captured_at").eq("client_id", client_id).order("week_start", { ascending: false }).limit(weeks ?? 8);
    if (weekError) {
      return { content: [{ type: "text", text: weekError.message }], isError: true };
    }
    let posts = [];
    if (include_posts !== false) {
      const { data: postRows, error: postError } = await sb.from("social_post_metrics").select("media_id, media_type, caption, permalink, posted_at, like_count, comments_count, reach, saved, shares, total_interactions").eq("client_id", client_id).order("posted_at", { ascending: false }).limit(25);
      if (postError) {
    ...
    return {
      content: [{
        type: "text",
        text: `${weekRows?.length ?? 0} semanas de m\xE9tricas e ${posts.length} publica\xE7\xF5es.`
      }],
...
var register_client_update_default = defineTool10({
  ...
  inputSchema: {
    client_id: z9.string().uuid().describe("Cliente dono do diario."),
    message: z9.string().min(5).max(4e3).describe("O que foi feito, por que foi feito e qual o proximo passo."),
    client_visible: z9.boolean().optional().describe("true (padrao) = cliente ve; false = nota interna.")
  },
...
var get_client_journal_default = defineTool11({
  ...
  inputSchema: {
    client_id: z10.string().uuid().describe("Cliente."),
    limit: z10.number().int().min(1).max(100).optional().describe("Maximo de itens por fonte (padrao 25).")
  },
  annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false },
  handler: async ({ client_id, limit }, ctx) => {
    ...
    const [notes, files, publications, reports] = await Promise.all([
      projectIds.length ? sb.from("updates").select("message, update_type, client_visible, created_at").in("project_id", projectIds).order("created_at", { ascending: false }).limit(max) : Promise.resolve({ data: [] }),
...
var create_project_default = defineTool12({
  name: "create_project",
  title: "Criar projeto",
  ...
  inputSchema: {
    client_id: z11.string().uuid().describe("Cliente dono do projeto."),
    name: z11.string().min(1).max(200).describe("Nome do projeto."),
    project_type: z11.string().min(1).max(64).describe("Tipo (ex.: social_media, site, automacao, evento)."),
    start_date: z11.string().describe("Inicio, ISO (YYYY-MM-DD)."),
    deadline: z11.string().describe("Prazo, ISO (YYYY-MM-DD)."),
    description: z11.string().max(8e3).optional(),
    scope: z11.string().max(8e3).optional(),
    objectives: z11.string().max(8e3).optional()
  },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false },
  handler: async (input, ctx) => {
    const guard = requireAuth(ctx);
...
var CAMPOS = ["name", "description", "status", "project_type", "start_date", "deadline", "progress", "scope", "objectives"];
var update_project_default = defineTool13({
  ...
  inputSchema: {
    project_id: z12.string().uuid(),
    name: z12.string().min(1).max(200).optional(),
    description: z12.string().max(8e3).nullable().optional(),
    status: z12.enum(["planning", "active", "done", "paused", "standby", "cancelled"]).optional(),
    project_type: z12.string().min(1).max(64).optional(),
    start_date: z12.string().optional(),
    deadline: z12.string().optional(),
    progress: z12.number().int().min(0).max(100).optional(),
    scope: z12.string().max(8e3).nullable().optional(),
    objectives: z12.string().max(8e3).nullable().optional()
  },
...
var update_task_default = defineTool14({
  ...
  inputSchema: {
    ...
    description: z13.string().max(4e3).nullable().optional(),
    status: z13.enum(TASK_STATUS_VALUES).optional(),
    priority: z13.enum(["low", "medium", "high", "urgent"]).optional(),
    delivery_type: z13.enum(TASK_DELIVERY_TYPE_VALUES).optional(),
    due_date: z13.string().nullable().optional().describe("ISO date (YYYY-MM-DD) ou null para limpar."),
    assigned_to: z13.string().uuid().nullable().optional()
  },
  annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  handler: async (input, ctx) => {
    const guard = requireAuth(ctx);
...
var complete_task_default = defineTool15({
  name: "complete_task",
...
// src/lib/mcp/index.ts
var supabaseUrl = String(define_import_meta_env_default.VITE_SUPABASE_URL ?? "").replace(/\/$/, "");
