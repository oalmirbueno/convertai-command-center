...
function outputPath(name) {
  assert(/^\d{14}_ci_(bind_approved_editorial|restore_editorial_names)\.sql$/.test(name),"Unexpected staged filename");
...
async function main() {
  ...
  if (command==="--self-test") {
    ...
    assert.equal(new Set(plan.files.map(f=>f.name)).size,4);
  ...
  if (command==="--stage") {
    ...
    console.log("Staged four CI-only adapters; historical migrations unchanged.");
    return;
  }
  if (command==="--restore") {
    requireCI();
    ...
    assert(Array.isArray(state) && state.length===4,"Invalid CI staging record");
    ...
    console.log("Removed only the four hash-verified generated CI adapter files.");
    return;
  }
